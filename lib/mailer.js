...
function fromAddr() {
  const name = process.env.SMTP_FROM_NAME || 'Direktorat Kursus dan Pelatihan'
...
export function otpEmail({ code, expiresMinutes = 15, name = '' }) {
  const brand = '#0B2545'
  const accent = '#1D4ED8'
  const subject = 'Kode Verifikasi Reset Kata Sandi — Direktorat Kursus dan Pelatihan'
      ...
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 2px 8px rgba(15,23,42,0.06)">
        <tr>
          <td style="background:linear-gradient(135deg,${brand} 0%,${accent} 100%);padding:24px 32px;color:#ffffff">
            ...
            <div style="font-size:20px;font-weight:700;margin-top:4px">Direktorat Kursus dan Pelatihan</div>
        ...
        <tr>
          <td style="padding:32px 32px 8px">
            ...
            <p style="margin:12px 0 0;color:#475569;font-size:14px;line-height:1.6">
              Halo${name?' <strong>'+name+'</strong>':''}, kami menerima permintaan reset kata sandi untuk akun Anda di Dashboard Direktorat Kursus dan Pelatihan.
        ...
        <tr>
          <td style="padding:20px 32px;background:#F8FAFC;border-top:1px solid #E2E8F0">
            <p style="margin:0;font-size:11px;color:#64748B;line-height:1.6">
              Email ini dikirim otomatis oleh sistem Dashboard Media Sosial Direktorat Kursus dan Pelatihan.<br>
