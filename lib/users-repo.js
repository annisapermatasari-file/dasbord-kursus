...
const PUBLIC_COLUMNS = 'name, business_name, email, role, plan, org_owner_email, jabatan, initial, active, seeded, created_at'
...
function toDoc(row) {
  ...
  return {
    ...
    // Setiap workspace = 1 Admin yang self-register + tim yang ia undang.
    // Baris lama (sebelum kolom ini ada) dianggap pemilik workspace sendiri.
    orgOwnerEmail: row.org_owner_email || row.email,
    jabatan: row.jabatan || '',
    initial: row.initial,
...
/** Daftar user dalam satu workspace (owner + tim yang ia undang). */
export async function listUsers(orgOwnerEmail) {
  let query = supabase().from(TABLE).select(PUBLIC_COLUMNS).order('created_at', { ascending: true })
  if (orgOwnerEmail) query = query.eq('org_owner_email', orgOwnerEmail)
...
/** Jumlah Admin aktif selain `email` DALAM workspace yang sama — dipakai untuk menjaga Admin terakhir per workspace. */
export async function countOtherActiveAdmins(email, orgOwnerEmail) {
  const { count, error } = await supabase()
    .from(TABLE)
    .select('email', { count: 'exact', head: true })
    .eq('role', 'Admin')
    .eq('active', true)
    .eq('org_owner_email', orgOwnerEmail)
    .neq('email', email)
 ...
 */
export async function upsertUser({ name, businessName, email, password, role, plan, orgOwnerEmail, jabatan, initial, active }) {
  const { data, error } = await supabase()
    .from(TABLE)
    .upsert(
      {
        ...
        role,
        plan: plan || 'starter',
        org_owner_email: orgOwnerEmail || email,
        jabatan: jabatan || '',
        initial,
        active,
