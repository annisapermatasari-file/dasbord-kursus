...
const PUBLIC_COLUMNS = 'name, business_name, email, role, jabatan, initial, active, seeded, created_at'
 ...
 */
export async function upsertUser({ name, businessName, email, password, role, jabatan, initial, active }) {
  const { data, error } = await supabase()
    .from(TABLE)
    .upsert(
      {
