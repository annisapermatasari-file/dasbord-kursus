...
/* ================= History (per-post feed with metrics) ================= */
// Ayrshare /history endpoint — returns posts published via Ayrshare with per-post metrics
// Docs: https://www.ayrshare.com/docs/apis/history/overview
export async function history(profileKey, { lastRecords = 100, lastDays } = {}) {
  const qs = new URLSearchParams()
  if (lastRecords) qs.set('lastRecords', String(lastRecords))
  if (lastDays) qs.set('lastDays', String(lastDays))
  const path = qs.toString() ? `/history?${qs}` : '/history'
  return ayrFetch(path, { method: 'GET', profileKey })
}

/* ================= Local Mongo helpers ================= */
// One Ayrshare profile per workspace. `key` is the workspace's org_owner_email
// (see actorWorkspace() in the API route) so each client's connected social
// accounts stay isolated from every other client.
