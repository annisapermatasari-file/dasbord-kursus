...
const PRESET_USERS = [
  { name:'Andi Prakoso',    role:'Admin',     initial:'AP', jabatan:'Kepala Sub-Bagian Humas' },
  { name:'Rina Setiawati',  role:'Analyst',   initial:'RS', jabatan:'Analis Komunikasi Digital' },
  { name:'Budi Santosa',    role:'Executive', initial:'BS', jabatan:'Direktur Kursus dan Pelatihan' },
  { name:'Dewi Rahayu',     role:'Viewer',    initial:'DR', jabatan:'Staf Publikasi' },
]
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-slate-100 flex items-center justify-center p-6">
      <div className="max-w-4xl w-full bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden grid grid-cols-1 md:grid-cols-2">
        <div className="bg-gradient-to-br from-[#0B2545] to-[#1D4ED8] text-white p-10 flex flex-col justify-between">
          <div>
            <div className="w-16 h-16 rounded-2xl bg-white/10 flex items-center justify-center ring-1 ring-white/20 mb-6"><ShieldCheck className="w-9 h-9" /></div>
            <div className="text-[11px] uppercase tracking-[0.25em] text-blue-200">Republik Indonesia</div>
            <h1 className="text-2xl font-bold mt-2 leading-tight">Dashboard Media Sosial</h1>
            <div className="text-lg font-semibold text-blue-200">Direktorat Kursus dan Pelatihan</div>
            <p className="text-sm text-blue-100/80 mt-4 leading-relaxed">Monitoring, Analisis, dan Evaluasi Komunikasi Digital untuk mendukung <em>Data-driven Communication for Education</em>.</p>
          </div>
          <div className="text-xs text-blue-200/70 pt-6 border-t border-white/10">© {new Date().getFullYear()} Kementerian Pendidikan Dasar dan Menengah</div>
        </div>
        <div className="p-10">
          <div className="text-[11px] uppercase tracking-widest text-slate-500 font-semibold">Masuk sebagai</div>
          <h2 className="text-xl font-bold text-slate-900 mt-1">Pilih Peran Pengguna</h2>
          <p className="text-sm text-slate-500 mt-1">Akses menu akan disesuaikan dengan peran Anda.</p>
          <div className="mt-6 space-y-2.5">
            {PRESET_USERS.map(u => { const role = ROLES[u.role]; return (
              <button key={u.name} onClick={()=>onLogin(u)} className="w-full flex items-center gap-3 p-3 rounded-xl border border-slate-200 hover:border-blue-400 hover:bg-blue-50/40 transition text-left group">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white font-bold text-sm shrink-0" style={{ background: role.color }}>{u.initial}</div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2"><span className="font-semibold text-slate-900">{u.name}</span><span className="text-[10px] px-1.5 py-0.5 rounded font-semibold text-white" style={{ background: role.color }}>{u.role}</span></div>
                  <div className="text-xs text-slate-500 mt-0.5">{u.jabatan}</div>
                  <div className="text-[11px] text-slate-400 mt-1 leading-relaxed">{role.desc}</div>
                </div>
                <ChevronRight className="w-4 h-4 text-slate-400 group-hover:text-blue-600" />
              </button>
            )})}
          </div>
          <div className="mt-6 text-[11px] text-slate-400 text-center">Demo login · Integrasi SSO Kemdikbud dapat diaktifkan pada Settings</div>
        </div>
      </div>
    </div>
  )
